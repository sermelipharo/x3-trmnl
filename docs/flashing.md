...
1. Build `firmware.bin` (`pio run`) if you don't already have one.
2. Connect the X3 over USB-C.
3. Chrome/Edge/Arc → <https://x3.crosspointreader.com>.
4. *Flash from file* → select `.pio/build/default/firmware.bin`.
5. The tool writes to the inactive OTA slot and flips `otadata` so the
   device boots into the new image — no manual partition fiddling.
