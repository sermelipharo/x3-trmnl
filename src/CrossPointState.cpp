...
bool CrossPointState::loadFromBinaryFile() {
  ...
  // Legacy fields from reader-era format: consume and discard so existing
  // state files still load without spurious errors.
  std::string legacyOpenEpubPath;
  serialization::readString(inputFile, legacyOpenEpubPath);
  if (version >= 2) {
  ...
  if (version >= 3) {
    uint8_t legacyReaderLoadCount = 0;
    serialization::readPod(inputFile, legacyReaderLoadCount);
  }
  if (version >= 4) {
    bool legacyLastSleepFromReader = false;
    serialization::readPod(inputFile, legacyLastSleepFromReader);
  }
